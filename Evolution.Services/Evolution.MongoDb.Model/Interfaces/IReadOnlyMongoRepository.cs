...
namespace Evolution.MongoDb.Model.Interfaces
{
    public interface IReadOnlyMongoRepository
    {
        ...
        Task<List<TDocument>> GetAllAsync<TDocument>(FilterDefinition<TDocument> filter, string partitionKey = null)
            where TDocument : IDocument;

        Task<List<TField>> GetDistinctAsync<TDocument, TField>(FieldDefinition<TDocument, TField> field, FilterDefinition<TDocument> filter, string partitionKey = null)
           where TDocument : IDocument
           where TField : class;
