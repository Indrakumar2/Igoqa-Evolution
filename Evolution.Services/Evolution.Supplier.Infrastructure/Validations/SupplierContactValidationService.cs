...
{
    public class SupplierContactValidationService : ISupplierContactValidationService
    {
        private IValidationService _validation = null;
        private const string _expected = "expected";
        private const string _actual = "actual";

        public SupplierContactValidationService(IValidationService validation)
        {
        ...
        public IList<JsonPayloadValidationResult> Validate(string jsonModel, ValidationType type)
        {
            ...
            if (type == ValidationType.Add)
                basePath = Path.Combine(basePath, "Save", "SupplierContactAdd.json");
            else if (type == ValidationType.Update)
                basePath = Path.Combine(basePath, "Update", "SupplierContactUpdate.json");
            else if (type == ValidationType.Delete)
                basePath = Path.Combine(basePath, "Delete", "SupplierContactDelete.json");
            else
