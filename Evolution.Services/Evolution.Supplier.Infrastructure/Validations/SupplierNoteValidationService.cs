...
namespace Evolution.Supplier.Infrastructure.Validations
{
    public class SupplierNoteValidationService : ISupplierNoteValidationService
    {
        private IValidationService _validation = null;
        private const string _expected = "expected";
        private const string _actual = "actual";

        public SupplierNoteValidationService(IValidationService validation)
        {
        ...
        public IList<JsonPayloadValidationResult> Validate(string jsonModel, ValidationType type)
        {
            ...
            string basePath = Path.Combine(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location), "JsonValidationSchemas", "Supplier");

            if (type == ValidationType.Add)
                basePath = Path.Combine(basePath, "Save", "SupplierNoteAdd.json");
            if (type == ValidationType.Delete)
                basePath = Path.Combine(basePath, "Delete", "SupplierNoteDelete.json");
            if(type == ValidationType.Update)       //D661 issue 8 
                basePath = Path.Combine(basePath, "Update", "SupplierNoteUpdate.json");
            else
