...
namespace Evolution.Supplier.Infrastructure.Validations
{
    public class SupplierValidationService : ISupplierValidationService
    {
        ...
        public SupplierValidationService(IValidationService validation)
        {
        ...
        public IList<JsonPayloadValidationResult> Validate(string jsonModel, ValidationType type)
        {
            ...
            if (type == ValidationType.Add)
                basePath = Path.Combine(basePath, "Save", "SupplierAdd.json");
            else if (type == ValidationType.Update)
                basePath = Path.Combine(basePath, "Update", "SupplierUpdate.json");
            else if (type == ValidationType.Delete)
                basePath = Path.Combine(basePath, "Delete", "SupplierDelete.json");
            else
