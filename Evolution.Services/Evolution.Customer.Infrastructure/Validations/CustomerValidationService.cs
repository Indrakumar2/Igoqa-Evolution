...
namespace Evolution.Customer.Infrastructure.Validations
{
    public class CustomerValidationService : ICustomerValidationService
    {
        ...
        public CustomerValidationService(IValidationService validation) { this._validation = validation; }
        ...
        public IList<JsonPayloadValidationResult> Validate(string jsonModel, ValidationType type)
        {
            ...
            if (type == ValidationType.Add)
                path = Path.Combine(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location), "JsonValidationSchemas", "Customer", "Save", "Customer.json");
            else
                path = Path.Combine(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location), "JsonValidationSchemas", "Customer", "Update", "Customer.json");
