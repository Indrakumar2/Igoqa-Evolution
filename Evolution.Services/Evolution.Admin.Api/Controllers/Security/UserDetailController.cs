...
using Evolution.Admin.Api.Filters;
using Evolution.Admin.Api.Models;
...
namespace Evolution.Admin.Api.Controllers.Security
{
    ...
    {
        private readonly IAppLogger<UserDetailController> _logger;
        private readonly JObject _messages;
        ...
        public Response GetUserPermission([FromRoute] string userSamaName, [FromQuery] string module,
            ...
            try
            {
                if (model != null)
                {
                    model = model.Where(x => x.User != null)
                        .Select(x =>
                        {
                            x.User.ApplicationName = ApplicationName;
                            return x;
                        }).ToList();
                ...
                return new Response().ToPopulate(ResponseType.Success, null, null,
                    new List<ValidationMessage>
                    {
                        new ValidationMessage(model,
                            new List<MessageDetail>
                            {
                                new MessageDetail(ModuleType.Security, MessageType.InvalidPayLoad.ToId(),
                                    _messages[MessageType.InvalidPayLoad.ToId()].ToString())
                            })
                    }, null, null);
            }
            catch (Exception ex)
        ...
        public Response Put([FromBody] UserStatusModel statusModel)
        {
            ...
            try
            {
                if (statusModel == null) statusModel = new UserStatusModel();
