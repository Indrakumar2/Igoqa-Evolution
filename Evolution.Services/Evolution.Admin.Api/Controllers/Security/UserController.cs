...
namespace Evolution.Admin.Api.Controllers.Security
{
    ...
    public class UserController : BaseController
    {
        ...
        [HttpGet]
        public Response Get([FromQuery] Model.UserInfo model, [FromQuery] string[] excludeUserTypes,
            [FromQuery] bool isGetAllUser = false)
        {
            Exception exception = null;
            ResponseType responseType = ResponseType.Success;
            try
            {
                return _userService.Get(model, excludeUserTypes, isGetAllUser);
            }
            catch (Exception ex)
            {
                exception = ex;
        ...
        [Route("BasicInfo")]
        [HttpGet]
        public Response Get([FromQuery] string companyCode, [FromQuery] string userType,
            [FromQuery] bool isActive = true)
        {
            ...
            try
            {
                return _userService.GetUser(companyCode, userType, isActive);
            }
            catch (Exception ex)
            {
                exception = ex;
                ...
                _logger.LogError(ResponseType.Exception.ToId(), ex.ToFullString(),
                    new {companyCode, userType, isActive});
            }
        ...
        public Response GetViewAllAssignments([FromQuery] string samAccountName)
        {
            ...
            try
            {
                return _userService.GetViewAllAssignments(samAccountName);
            }
            catch (Exception ex)
            {
                exception = ex;
        ...
        public Response Get([FromRoute] string userName, [FromRoute] string menuname)
        {
            ...
            try
            {
                return _userService.GetUserCompanyRoles(userName, menuname);
            }
            catch (Exception ex)
            {
                exception = ex;
