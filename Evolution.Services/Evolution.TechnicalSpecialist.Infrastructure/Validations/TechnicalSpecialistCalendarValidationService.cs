...
{
    public class TechnicalSpecialistCalendarValidationService: ITechnicalSpecialistCalendarValidationService
    {
        private IValidationService _validation = null;
        private const string _expected = "expected";
        private const string _actual = "actual";

        public TechnicalSpecialistCalendarValidationService(IValidationService validation)
        {
        ...
        public IList<JsonPayloadValidationResult> Validate(string jsonModel, ValidationType type)
        {
            ...
            if (type == ValidationType.Add)
                basePath = Path.Combine(basePath, "Save", "TechnicalSpecialistCalendarAdd.json");
            else if (type == ValidationType.Update)
                basePath = Path.Combine(basePath, "Update", "TechnicalSpecialistCalendarUpdate.json");
            else if (type == ValidationType.Delete)
                basePath = Path.Combine(basePath, "Delete", "TechnicalSpecialistCalendarDelete.json");
            else
