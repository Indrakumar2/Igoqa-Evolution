...
{
    public class TechnicalSpecialistTrainingAndCompetencyTypeValidationService : ITechnicalSpecialistTrainingAndCompetencyTypeValidationService
    {
        private IValidationService _validation = null;
        private const string _expected = "expected";
        private const string _actual = "actual";

        public TechnicalSpecialistTrainingAndCompetencyTypeValidationService(IValidationService validation) { this._validation = validation; }
         ...
         public IList<JsonPayloadValidationResult> Validate(string jsonModel, ValidationType type)
        {
            ...
            if (type == ValidationType.Add)
                jsonValidationSchemaPath = Path.Combine(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location), "JsonValidationSchemas", "TechnicalSpecialist", "Save", "TechnicalSpecialistTrainingAndCompetencyType.json");
            else if (type == ValidationType.Update)
                jsonValidationSchemaPath = Path.Combine(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location), "JsonValidationSchemas", "TechnicalSpecialist", "Update", "TechnicalSpecialistTrainingAndCompetencyType.json");
            else if (type == ValidationType.Delete)
                jsonValidationSchemaPath = Path.Combine(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location), "JsonValidationSchemas", "TechnicalSpecialist", "Delete", "TechnicalSpecialistTrainingAndCompetencyType.json");
            else
