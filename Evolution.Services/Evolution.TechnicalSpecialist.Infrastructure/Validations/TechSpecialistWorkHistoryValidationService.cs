...
{
    public class TechSpecialistWorkHistoryValidationService: ITechnicalSpecialistWorkHistoryValidationService
    {
        private IValidationService _validation = null;
        private const string _expected = "expected";
        private const string _actual = "actual";

        public TechSpecialistWorkHistoryValidationService(IValidationService validation)
        {
        ...
        public IList<JsonPayloadValidationResult> Validate(string jsonModel, ValidationType type)
        {
            ...
            if (type == ValidationType.Add)
                jsonSchemaValidationPath = Path.Combine(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location), "JsonValidationSchemas", "TechnicalSpecialist", "Save", "TechSpecialistWorkHistoryAdd.json");
            else if (type == ValidationType.Update)
                jsonSchemaValidationPath = Path.Combine(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location), "JsonValidationSchemas", "TechnicalSpecialist", "Update", "TechSpecialistWorkHistoryUpdate.json");
            else if (type == ValidationType.Delete)
                jsonSchemaValidationPath = Path.Combine(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location), "JsonValidationSchemas", "TechnicalSpecialist", "Delete", "TechSpecilaistWorkHistoryDelete.json");
            else
