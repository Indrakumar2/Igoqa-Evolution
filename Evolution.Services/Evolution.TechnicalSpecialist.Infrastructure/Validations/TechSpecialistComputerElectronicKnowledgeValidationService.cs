...
{
    public class TechSpecialistComputerElectronicKnowledgeValidationService: ITechnicalSpecialistComputerElectronicKnowledgeValidationService
    {
        private IValidationService _validation = null;
        private const string _expected = "expected";
        private const string _actual = "actual";

        public TechSpecialistComputerElectronicKnowledgeValidationService(IValidationService validation)
        {
        ...
        public IList<JsonPayloadValidationResult> Validate(string jsonModel, ValidationType type)
        {
            ...
            if (type == ValidationType.Add)
                basePath = Path.Combine(basePath, "Save", "TechSpecialComputerElectronicKnowledgeAdd.json");
            else if (type == ValidationType.Update)
                basePath = Path.Combine(basePath, "Update", "TechSpecialistComputerElectronicKnowledgeUpdate.json");
            else if (type == ValidationType.Delete)
                basePath = Path.Combine(basePath, "Delete", "TechSpecialistComputerElectronicKnowledgeDelete.json");
            else
