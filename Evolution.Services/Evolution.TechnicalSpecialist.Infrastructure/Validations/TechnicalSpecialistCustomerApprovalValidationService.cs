...
{
    public class TechnicalSpecialistCustomerApprovalValidationService : ITechnicalSpecialistCustomerApprovalValidationService
    {
        private IValidationService _validation = null;
        private const string _expected = "expected";
        private const string _actual = "actual";

        public TechnicalSpecialistCustomerApprovalValidationService(IValidationService validation)
        {
        ...
        { 
            ...
            if (type == ValidationType.Add)
                basePath = Path.Combine(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location), "JsonValidationSchemas", "TechnicalSpecialist", "Save", "TechSpecialistCustomerApprovalAdd.json");
            else if (type == ValidationType.Update)
                basePath = Path.Combine(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location), "JsonValidationSchemas", "TechnicalSpecialist", "Update", "TechnicalSpecialistCustomerApprovalUpdate.json");
            else if (type == ValidationType.Delete)
                basePath = Path.Combine(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location), "JsonValidationSchemas", "TechnicalSpecialist", "Delete", "TechnicalSpecialistCustomerApprovalDelete.json");
            else
