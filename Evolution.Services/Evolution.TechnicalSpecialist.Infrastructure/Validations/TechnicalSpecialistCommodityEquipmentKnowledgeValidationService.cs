...
{
    public class TechnicalSpecialistCommodityEquipmentKnowledgeValidationService: ITechnicalSpecialistCommodityEquipmentKnowledgeValidationService
    {
        private IValidationService _validation = null;
        private const string _expected = "expected";
        private const string _actual = "actual";

        public TechnicalSpecialistCommodityEquipmentKnowledgeValidationService(IValidationService validation)
        {
        ...
        public IList<JsonPayloadValidationResult> Validate(string jsonModel, ValidationType type)
        {
            ...
            if (type == ValidationType.Add)
                basePath = Path.Combine(basePath, "Save", "TechnicalSpecialistCommodityEquipmentKnowledgeAdd.json");
            else if (type == ValidationType.Update)
                basePath = Path.Combine(basePath, "Update", "TechnicalSpecialistCommodityEquipmentKnowledgeUpdate.json");
            else if (type == ValidationType.Delete)
                basePath = Path.Combine(basePath, "Delete", "TechnicalSpecialistCommodityEquipmentKnowledgeDelete.json");
            else
