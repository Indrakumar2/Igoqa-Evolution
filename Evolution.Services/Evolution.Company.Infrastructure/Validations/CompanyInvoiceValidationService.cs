...
{
    public class CompanyInvoiceValidationService : ICompanyInvoiceTextValidationService
    {
        private IValidationService _validation = null;
        private const string _expected = "expected";
        private const string _actual = "actual";

        public CompanyInvoiceValidationService(IValidationService validation) { this._validation = validation; }
        ...
        public IList<JsonPayloadValidationResult> Validate(string jsonModel, ValidationType type)
        {
            ...
            if (type == ValidationType.Add)
                path = Path.Combine(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location), "JsonValidationSchemas", "Company", "Save", "CompanyInvoiceTextAdd.json");
            if (type == ValidationType.Update)
                path = Path.Combine(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location), "JsonValidationSchemas", "Company", "Update", "CompanyInvoiceTextUpdate.json");
            if (type == ValidationType.Delete)
                path = Path.Combine(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location), "JsonValidationSchemas", "Company", "Delete", "CompanyInvoiceTextDelete.json");
