...
namespace Evolution.Company.Infrastructe.Validations
{
    /// <summary>
    /// This will responsible for validate Company Payroll Model
    /// </summary>
    public class CompanyPayrollValidationService : ICompanyPayrollValidationService
    {
        private IValidationService _validation = null;
        private const string _expected = "expected";
        private const string _actual = "actual";

        public CompanyPayrollValidationService(IValidationService validation) { this._validation = validation; }
        ...
        public IList<JsonPayloadValidationResult> Validate(string jsonModel, ValidationType type)
        {
            ...
            if (type == ValidationType.Add)
                path = Path.Combine(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location), "JsonValidationSchemas", "Company", "Save", "CompanyPayrollAdd.json");
            if (type == ValidationType.Update)
                path = Path.Combine(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location), "JsonValidationSchemas", "Company", "Update", "CompanyPayrollUpdate.json");
            if (type == ValidationType.Delete)
                path = Path.Combine(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location), "JsonValidationSchemas", "Company", "Delete", "CompanyPayrollDelete.json");
