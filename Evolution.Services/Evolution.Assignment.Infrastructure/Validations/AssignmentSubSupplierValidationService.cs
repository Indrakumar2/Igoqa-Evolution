...
{
    public class AssignmentSubSupplierValidationService : IAssignmentSubSupplierValidationService
    {
        ...
        public AssignmentSubSupplierValidationService(IValidationService validation) { this._validation = validation; }
        ...
        public IList<JsonPayloadValidationResult> Validate(string jsonModel, ValidationType type)
        {
            ...
            if (type == ValidationType.Add)
                basePath = Path.Combine(basePath, "Save", "AssignmentSubSupplierAdd.json");
            else if (type == ValidationType.Update)
                basePath = Path.Combine(basePath, "Update", "AssignmentSubSupplierUpdate.json");
            else if (type == ValidationType.Delete)
                basePath = Path.Combine(basePath, "Delete", "AssignmentSubSupplierDelete.json");
            else
