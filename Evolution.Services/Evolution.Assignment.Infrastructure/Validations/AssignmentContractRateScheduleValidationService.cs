...
{
    public class AssignmentContractRateScheduleValidationService : IAssignmentContractRateScheduleValidationService
    {
        ...
        public AssignmentContractRateScheduleValidationService(IValidationService validation) { this._validation = validation; }
        ...
        public IList<JsonPayloadValidationResult> Validate(string jsonModel, ValidationType type)
        {
            ...
            if (type == ValidationType.Add)
                basePath = Path.Combine(basePath, "Save", "AssignmentContractScheduleAdd.json");
            else if (type == ValidationType.Update)
                basePath = Path.Combine(basePath, "Update", "AssignmentContractScheduleUpdate.json");
            else if (type == ValidationType.Delete)
                basePath = Path.Combine(basePath, "Delete", "AssignmentContractScheduleDelete.json");
            else
