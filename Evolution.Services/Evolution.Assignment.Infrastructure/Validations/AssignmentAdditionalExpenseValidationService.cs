...
{
    public class AssignmentAdditionalExpenseValidationService: IAssignmentAdditionalExpenseValidationService
    {
        ...
        public AssignmentAdditionalExpenseValidationService(IValidationService validation) { this._validation = validation; }
        ...
        public IList<JsonPayloadValidationResult> Validate(string jsonModel, ValidationType type)
        {
            ...
            if (type == ValidationType.Add)
                basePath = Path.Combine(basePath, "Save", "AssignmentAdditionalExpenseAdd.json");
            else if (type == ValidationType.Update)
                basePath = Path.Combine(basePath, "Update", "AssignmentAdditionalExpenseUpdate.json");
            else if (type == ValidationType.Delete)
                basePath = Path.Combine(basePath, "Delete", "AssignmentAdditionalExpenseDelete.json");
            else
