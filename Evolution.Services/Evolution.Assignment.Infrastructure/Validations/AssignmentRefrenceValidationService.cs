...
{
    public class AssignmentRefrenceValidationService : IAssignmentReferenceTypeValidationService
    {
        ...
        public AssignmentRefrenceValidationService(IValidationService validation) { this._validation = validation; }
        ...
        public IList<JsonPayloadValidationResult> Validate(string jsonModel, ValidationType type)
        {
            ...
            string basePath = System.IO.Path.Combine(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location), "JsonValidationSchemas", "Assignment");
            string jsonSchema = string.Empty;
            var moduleValidationResults = new List<JsonPayloadValidationResult>();

            if (type == ValidationType.Add)
                basePath = Path.Combine(basePath, "Save", "AssignmentReferenceAdd.json");
            else if (type == ValidationType.Update)
                basePath = Path.Combine(basePath, "Update", "AssignmentRefrenceUpdate.json");
            else if (type == ValidationType.Delete)
                basePath = Path.Combine(basePath, "Delete", "AssignmentReferenceDelete.json");
            else
