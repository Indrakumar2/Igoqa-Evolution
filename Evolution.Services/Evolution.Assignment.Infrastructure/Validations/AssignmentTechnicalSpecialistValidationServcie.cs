...
{
    public  class AssignmentTechnicalSpecialistValidationServcie :IAssignmentTechnicalSpecilaistValidationService
    {
        ...
        private readonly IAppLogger<AssignmentTechnicalSpecialistValidationServcie> _logger = null;

        public AssignmentTechnicalSpecialistValidationServcie(IValidationService validation,
                                                              IAppLogger<AssignmentTechnicalSpecialistValidationServcie> logger)
        {
        ...
        public IList<JsonPayloadValidationResult> Validate(string jsonModel, ValidationType type)
        {
            ...
            if (type == ValidationType.Add)
                basePath = Path.Combine(basePath, "Save", "AssignmentTechnicalSpecialistAdd.json");
            else if (type == ValidationType.Update)
                basePath = Path.Combine(basePath, "Update", "AssignmentTechnicalSpecialistUpdate.json");
            else if (type == ValidationType.Delete)
                basePath = Path.Combine(basePath, "Delete", "AssignmentTechnicalSpecialistDelete.json");
            else
