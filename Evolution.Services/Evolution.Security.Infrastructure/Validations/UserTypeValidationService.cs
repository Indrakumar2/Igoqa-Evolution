...
{
    public class UserTypeValidationService : IUserTypeValidationService
    {
        private IValidationService _validation = null;
        private const string _expected = "expected";
        private const string _actual = "actual";

        public UserTypeValidationService(IValidationService validation) { this._validation = validation; }
        ...
        public IList<JsonPayloadValidationResult> Validate(string jsonModel, ValidationType type)
        {
            ...
            if (type == ValidationType.Add)
                basePath = Path.Combine(basePath, "Save", "UserTypeAdd.json");
            else if (type == ValidationType.Update)
                basePath = Path.Combine(basePath, "Update", "UserTypeUpdate.json");
            else if (type == ValidationType.Delete)
                basePath = Path.Combine(basePath, "Delete", "UserTypeDelete.json");
            else
