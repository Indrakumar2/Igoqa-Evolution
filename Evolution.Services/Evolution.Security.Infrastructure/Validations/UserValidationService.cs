...
namespace Evolution.Security.Infrastructure.Validations
{
    public class UserValidationService : IUserValidationService
    {
        ...
        public UserValidationService(IValidationService validation) { this._validation = validation; }
        ...
        public IList<JsonPayloadValidationResult> Validate(string jsonModel, ValidationType type)
        {
            ...
            if (type == ValidationType.Add)
                basePath = Path.Combine(basePath, "Save", "UserAdd.json");
            else if (type == ValidationType.Update)
                basePath = Path.Combine(basePath, "Update", "UserUpdate.json");
            else if (type == ValidationType.Delete)
                basePath = Path.Combine(basePath, "Delete", "UserDelete.json");
            else
