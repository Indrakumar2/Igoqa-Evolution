...
namespace Evolution.Security.Infrastructure.Validations
{
    public class UserRoleValidationService : IUserRoleValidationService
    {
        ...
        public UserRoleValidationService(IValidationService validation) { this._validation = validation; }
        ...
        public IList<JsonPayloadValidationResult> Validate(string jsonModel, ValidationType type)
        {
            ...
            if (type == ValidationType.Add)
                basePath = Path.Combine(basePath, "Save", "UserRoleAdd.json");
            else if (type == ValidationType.Update)
                basePath = Path.Combine(basePath, "Update", "UserRoleUpdate.json");
            else if (type == ValidationType.Delete)
                basePath = Path.Combine(basePath, "Delete", "UserRoleDelete.json");
            else
