...
namespace Evolution.Security.Infrastructure.Validations
{
    public class RoleValidationService: IRoleValidationService
    {
        ...
        public RoleValidationService(IValidationService validation) { this._validation = validation; }
        ...
        public IList<JsonPayloadValidationResult> Validate(string jsonModel, ValidationType type)
        {
            ...
            if (type == ValidationType.Add)
                basePath = Path.Combine(basePath, "Save", "RoleAdd.json");
            else if (type == ValidationType.Update)
                basePath = Path.Combine(basePath, "Update", "RoleUpdate.json");
            else if (type == ValidationType.Delete)
                basePath = Path.Combine(basePath, "Delete", "RoleDelete.json");
            else
