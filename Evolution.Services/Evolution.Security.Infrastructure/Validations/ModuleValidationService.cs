...
namespace Evolution.Security.Infrastructure.Validations
{
    public class ModuleValidationService : IModuleValidationService
    {
        ...
        public ModuleValidationService(IValidationService validation) { this._validation = validation; }
        ...
        public IList<JsonPayloadValidationResult> Validate(string jsonModel, ValidationType type)
        {
            ...
            if (type == ValidationType.Add)
                basePath = Path.Combine(basePath, "Save", "ModuleAdd.json");
            else if (type == ValidationType.Update)
                basePath = Path.Combine(basePath, "Update", "ModuleUpdate.json");
            else if (type == ValidationType.Delete)
                basePath = Path.Combine(basePath, "Delete", "ModuleDelete.json");
            else
