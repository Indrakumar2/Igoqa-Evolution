...
{
    public class VisitSupplierPerformanceValidationService : IVisitSupplierPerformanceValidationService
    {
        ...
        public VisitSupplierPerformanceValidationService(IValidationService validation)
        {
        ...
        public IList<JsonPayloadValidationResult> Validate(string jsonModel, ValidationType type)
        {
            ...
            if (type == ValidationType.Add)
                basePath = Path.Combine(basePath, "Save", "VisitSupplierPerformanceAdd.json");
            if (type == ValidationType.Update)
                basePath = Path.Combine(basePath, "Update", "VisitSupplierPerformanceUpdate.json");
            if (type == ValidationType.Delete)
                basePath = Path.Combine(basePath, "Delete", "VisitSupplierPerformanceDelete.json");
