...
namespace Evolution.Visit.Infrastructure.Validations
{
    public class VisitValidationService : IVisitValidationService
    {
        ...
        public VisitValidationService(IValidationService validation)
        {
        ...
        public IList<JsonPayloadValidationResult> Validate(string jsonModel, ValidationType type)
        {
            ...
            if (type == ValidationType.Add)
                basePath = Path.Combine(basePath, "Save", "VisitAdd.json");
            else if (type == ValidationType.Update)
                basePath = Path.Combine(basePath, "Update", "VisitUpdate.json");
            else if (type == ValidationType.Delete)
                basePath = Path.Combine(basePath, "Delete", "VisitDelete.json");
            else
