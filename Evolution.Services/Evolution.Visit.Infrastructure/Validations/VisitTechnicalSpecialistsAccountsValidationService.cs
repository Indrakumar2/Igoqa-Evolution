...
{
    public class VisitTechnicalSpecialistsAccountsValidationService : IVisitTechnicalSpecialistAccountsValidationService
    {
        ...
        public VisitTechnicalSpecialistsAccountsValidationService(IValidationService validation)
        {
        ...
        public IList<JsonPayloadValidationResult> Validate(string jsonModel, ValidationType type)
        {
            ...
            if (type == ValidationType.Add)
                basePath = Path.Combine(basePath, "Save", "VisitTechSpecItemAdd.json");
            else if (type == ValidationType.Update)
                basePath = Path.Combine(basePath, "Update", "VisitTechSpecItemUpdate.json");
            else if (type == ValidationType.Delete)
                basePath = Path.Combine(basePath, "Delete", "VisitTechSpecItemDelete.json");
            else
