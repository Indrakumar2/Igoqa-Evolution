...
{
    class VisitTechnicalSpecialistAccountItemExpenseValidationService : IVisitTechnicalSpecialistAccountItemExpenseValidationService
    {
        ...
        public VisitTechnicalSpecialistAccountItemExpenseValidationService(IValidationService validation)
        {
        ...
        public IList<JsonPayloadValidationResult> Validate(string jsonModel, ValidationType type)
        {
            ...
            if (type == ValidationType.Add)
                basePath = Path.Combine(basePath, "Save", "VisitTechSpecItemExpenseAdd.json");
            else if (type == ValidationType.Update)
                basePath = Path.Combine(basePath, "Update", "VisitTechSpecItemExpenseUpdate.json");
            else if (type == ValidationType.Delete)
                basePath = Path.Combine(basePath, "Delete", "VisitTechSpecItemExpenseDelete.json");
            else
