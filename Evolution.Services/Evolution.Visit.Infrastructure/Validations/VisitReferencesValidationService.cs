...
{
    public class VisitReferencesValidationService : IVisitReferenceValidationService
    {
        ...
        public VisitReferencesValidationService(IValidationService validation)
        {
        ...
        public IList<JsonPayloadValidationResult> Validate(string jsonModel, ValidationType type)
        {
            ...
            if (type == ValidationType.Add)
                basePath = Path.Combine(basePath, "Save", "VisitReferenceAdd.json");
            else if (type == ValidationType.Update)
                basePath = Path.Combine(basePath, "Update", "VisitReferenceUpdate.json");
            else if (type == ValidationType.Delete)
                basePath = Path.Combine(basePath, "Delete", "VisitReferenceDelete.json");
            else
