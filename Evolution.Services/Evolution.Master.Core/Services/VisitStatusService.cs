...
{
    public class VisitStatusService : MasterService,IVisitStatusService
    {
        ...
        public VisitStatusService(IAppLogger<MasterService> logger, IMasterRepository repository, IMapper mapper,
        ...
        public Response Search(VisitStatus search)
        {
            IList<VisitStatus> result = null;
            ResponseType responseType = ResponseType.Success;
            try
            {
                if (search == null)
                    search = new VisitStatus();

                var masterData = _mapper.Map<VisitStatus, MasterData>(search);
                var cacheKey = "VisitStatus";
                if (!_memoryCache.TryGetValue(cacheKey, out result))
                {
                    using (var tranScope = new TransactionScope(TransactionScopeOption.RequiresNew, new TransactionOptions { IsolationLevel = IsolationLevel.ReadUncommitted }))
                    {
                        result = this._repository.FindBy(_mapper.Map<MasterData, Data>(masterData).ToExpression())
                                            .Where(x => x.MasterDataTypeId == Convert.ToInt32(MasterType.VisitStatus))
                                            .Select(x => _mapper.Map<MasterData, VisitStatus>(_mapper.Map<Data, MasterData>(x))).OrderBy(x => x.Name).ToList();
