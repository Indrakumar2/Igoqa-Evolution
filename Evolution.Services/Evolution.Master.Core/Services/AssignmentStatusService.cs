...
{
   public class AssignmentStatusService:MasterService,IAssignmentStatusService
    {
        ...
        public AssignmentStatusService(IMapper mapper,IAppLogger<MasterService> logger, 
        ...
        public Response Search(AssignmentStatus search)
        {
            IList<AssignmentStatus> result = null;
            ResponseType responseType = ResponseType.Success;
            try
            {
                if (search == null)
                    search = new AssignmentStatus();

                //var cacheKey = "AssignmentStatus";
                //if (!_memoryCache.TryGetValue(cacheKey, out result))
                //{
                    var masterData = _mapper.Map<AssignmentStatus, MasterData>(search);
                    using (var tranScope = new TransactionScope(TransactionScopeOption.RequiresNew, new TransactionOptions { IsolationLevel = IsolationLevel.ReadUncommitted }))
                    {
                        ...
                        if (search.Name.HasEvoWildCardChar() || search.Description.HasEvoWildCardChar())
                            result = this._repository.FindBy(_mapper.Map<MasterData, Data>(masterData).ToExpression())
                                                    .Where(x => x.MasterDataTypeId == Convert.ToInt32(MasterType.AssignmentStatus) && (search.IsActive == null || x.IsActive == search.IsActive))
                                                    .Select(x => _mapper.Map<MasterData, AssignmentStatus>(_mapper.Map<Data, MasterData>(x))).OrderBy(x => x.Name).ToList();
                        else
                        {
                            masterData.MasterDataTypeId = Convert.ToInt32(MasterType.AssignmentStatus);
                            result = _repository.GetMasterData(masterData).Select(x => _mapper.Map<MasterData, AssignmentStatus>(x)).OrderBy(x => x.Name).ToList();
                        }
