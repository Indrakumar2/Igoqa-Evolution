...
namespace Evolution.AuditLog.Infrastructure.Validations
{
    public class SqlAditLogDetailValidationService : ISqlAditLogDetailValidationService
    {
        private IValidationService _validation = null;
        private const string _expected = "expected";
        private const string _actual = "actual";

        public SqlAditLogDetailValidationService(IValidationService validation) { this._validation = validation; }
        ...
        public IList<JsonPayloadValidationResult> Validate(string jsonModel, ValidationType type)
        {
            ...
            string basePath = Path.Combine(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location), "JsonValidationSchemas", "SqlAuditLog");

            if (type == ValidationType.Add)
                basePath = Path.Combine(basePath, "Save", "SqlAditLogDetailAdd.json");
            else
