...
{
    public class SupplierPoSubSupplierValidationService : ISupplierPoSubSupplierValidationService
    {
        private IValidationService _validation = null;
        private const string _expected = "expected";
        private const string _actual = "actual";

        public SupplierPoSubSupplierValidationService(IValidationService validation) { this._validation = validation; }
        ...
        public IList<JsonPayloadValidationResult> Validate(string jsonModel, ValidationType type)
        {
            ...
            if (type == ValidationType.Add)
                basePath = Path.Combine(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location), "JsonValidationSchemas", "SupplierPO", "Save", "SupplierPOSubSupplier.json");
            if (type == ValidationType.Update)
                basePath = Path.Combine(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location), "JsonValidationSchemas", "SupplierPO", "Update", "SupplierPOSubSupplier.json");
            if (type == ValidationType.Delete)
                basePath = Path.Combine(Path.GetDirectoryName(Assembly.GetExecutingAssembly().Location), "JsonValidationSchemas", "SupplierPO", "Delete", "SupplierPOSubSupplier.json");
