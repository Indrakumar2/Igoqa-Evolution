...
namespace Evolution.Api.Filters
{
    public class AuthorisationFilter : TypeFilterAttribute
    {
        public AuthorisationFilter(SecurityModule module, params SecurityPermission[] activity): base(typeof(AuthorizeActionFilter))
        {
    ...
    public class AuthorizeActionFilter : IAuthorizationFilter
    {
        ...
        public AuthorizeActionFilter(SecurityModule module, IUserDetailService userDetailService, params SecurityPermission[] activity)
        {
            _module = module;
        ...
        public void OnAuthorization(AuthorizationFilterContext context)
        {
            ...
            if (!isAuthorized)
            {
                context.Result= new ObjectResult("Access denied.") { StatusCode = 403 };
            }
        }
        ...
        private bool UserPermission(AuthorizationFilterContext context, SecurityModule module, List<string> activity)
        {
           ...
           return _userDetailService.GetUserPermission(Convert.ToInt16(context.HttpContext.Request.Headers["company_id"]),
        ...
        private void GetUserName(AuthorizationFilterContext context,out string userName)
        {
            if (context.HttpContext.Request.Headers.Keys.Contains(KEY_AUTHORIZATION))
            {
                ...
                userName = decodedToken?.Claims.FirstOrDefault(x => x.Type == JwtRegisteredClaimNames.UniqueName.ToString())?.Value;
            }
            else
                userName=null;
        }
