...
{
    public class TechSpecItemExpenseValidationService : ITechSpecItemExpenseValidationService
    {
        ...
        public TechSpecItemExpenseValidationService(IValidationService validation)
        {
        ...
        public IList<JsonPayloadValidationResult> Validate(string jsonModel, ValidationType type)
        {
            ...
            if (type == ValidationType.Add)
                basePath = Path.Combine(basePath, "Save", "TechSpecItemExpenseAdd.json");
            else if (type == ValidationType.Update)
                basePath = Path.Combine(basePath, "Update", "TechSpecItemExpenseUpdate.json");
            else if (type == ValidationType.Delete)
                basePath = Path.Combine(basePath, "Delete", "TechSpecItemExpenseDelete.json");
            else
