...
{
    public class TechSpecItemTimeValidationService :ITechSpecItemTimeValidationService
    {
        ...
        public TechSpecItemTimeValidationService(IValidationService validation)
        {
        ...
        public IList<JsonPayloadValidationResult> Validate(string jsonModel, ValidationType type)
        {
            ...
            if (type == ValidationType.Add)
                basePath = Path.Combine(basePath, "Save", "TechSpecItemTimeAdd.json");
            else if (type == ValidationType.Update)
                basePath = Path.Combine(basePath, "Update", "TechSpecItemTimeUpdate.json");
            else if (type == ValidationType.Delete)
                basePath = Path.Combine(basePath, "Delete", "TechSpecItemTimeDelete.json");
            else
