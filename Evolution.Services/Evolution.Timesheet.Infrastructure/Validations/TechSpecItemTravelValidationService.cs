...
{
    public class TechSpecItemTravelValidationService : ITechSpecItemTravelValidationService
    {
        ...
        public TechSpecItemTravelValidationService(IValidationService validation)
        {
        ...
        public IList<JsonPayloadValidationResult> Validate(string jsonModel, ValidationType type)
        {
            ...
            if (type == ValidationType.Add)
                basePath = Path.Combine(basePath, "Save", "TechSpecItemTravelAdd.json");
            else if (type == ValidationType.Update)
                basePath = Path.Combine(basePath, "Update", "TechSpecItemTravelUpdate.json");
            else if (type == ValidationType.Delete)
                basePath = Path.Combine(basePath, "Delete", "TechSpecItemTravelDelete.json");
            else
