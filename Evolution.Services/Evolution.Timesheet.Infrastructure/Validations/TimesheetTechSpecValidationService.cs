...
{
    public class TimesheetTechSpecValidationService :ITimesheetTechSpecValidationService
    {
        ...
        public TimesheetTechSpecValidationService(IValidationService validation)
        {
        ...
        public IList<JsonPayloadValidationResult> Validate(string jsonModel, ValidationType type)
        {
            ...
            if (type == ValidationType.Add)
                basePath = Path.Combine(basePath, "Save", "TimesheetTechSpecAdd.json");
            else if (type == ValidationType.Update)
                basePath = Path.Combine(basePath, "Update", "TimesheetTechSpecUpdate.json");
            else if (type == ValidationType.Delete)
                basePath = Path.Combine(basePath, "Delete", "TimesheetTechSpecDelete.json");
            else
