...
class Notes extends Component {
    ...
    notesSubmitHandler = (e) => {
        ...
        if (this.updatedData.note === undefined || this.updatedData.note === "") {
            IntertekToaster(localConstant.warningMessages.NOTE_IS_MANDATORY, 'warningToast supplierPoNoteReferenceTypeReq');
        }else if(editedData.supplierPONoteId){ //D661 issue8
            ...
            this.props.actions.EditSupplierPoNotesDetails(editedData);
            this.hideModal(e);
        }
        else{
        ...
        this.updatedData["supplierPoNoteId"] = Math.floor(Math.random() * (Math.pow(10, 5)));
        this.props.actions.AddSupplierPoNotesDetails(this.updatedData);
        this.updatedData = {};
        this.hideModal(e);
        }
    }
    ...
    render() {
        ...
        const SupplierPoNoteHeaderData=HeaderData;
        const { supplierPoNotesData } = this.props;
        bindAction(SupplierPoNoteHeaderData, "ViewSupplierPoNotes", this.viewRowHandler);
        return (
            <Fragment>
                <CustomModal modalData={modelData} />
                <div className="customCard">
                    <h6 className="bold col s3 pl-0 pr-3">{localConstant.notes.SUPPLIERPO_NOTES}</h6>
                    <CardPanel className="white lighten-4 black-text" colSize="s12">
                        <ReactGrid gridRowData={supplierPoNotesData}
                            gridColData={SupplierPoNoteHeaderData}
                            handlerChange={this.noteHangleChange}
                            paginationPrefixId={localConstant.paginationPrefixIds.supplierPONotes}
...
Notes.propTypes = {
    supplierPoNotesData: PropTypes.array
};
