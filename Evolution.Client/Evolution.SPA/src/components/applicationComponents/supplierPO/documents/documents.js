...
import { SupplierPODocHeaderData, VisitDocHeaderData } from './headerData';
...
import SupplierPODocumentMenuButtons from '../../../documents/documents';
    ...
    return (
        <Fragment>
            <CustomInput hasLabel={true}
                ...
                optionsList={props.supplierPODocumentTypeData}
                optionName='name'
                optionValue="name"
    ...
    return (
        <Fragment>
            <CustomInput hasLabel={true}
                ...
                name="documentType"
                id="documentType"
                defaultValue={props.editedSupplierPODocuments.documentType}
            />
            <CustomInput
                hasLabel={true}
                divClassName='col'
                ...
                name="documentName"
                disabled={true}
                defaultValue={props.editedSupplierPODocuments.documentName}
            />
            ...
            {/* <div className="row">
                <CustomInput
                    ...
                    colSize="s3 m4 mt-4"
                    checkedStatus={props.editedSupplierPODocuments.isVisibleToCustomer}
                    onChangeToggle={props.handlerChange}
                />
            </div> */}
        </Fragment>
...
class SupplierPoDocuments extends Component {
    constructor(props) {
        super(props);
        this.state = {
        ...
        const functionRefs = {};
        functionRefs["enableDocumentDownload"] = this.enableDocumentDownload;
        this.headerData = SupplierPODocHeaderData(functionRefs);       
    };
    enableDocumentDownload =()=>{
        const enableDownloadButton=(this.props.interactionMode || this.props.supplierPOViewMode );
        return enableDownloadButton;
    }
    editRowHandler = (data) => {
        this.setState({
    ...
    componentDidMount() {
        ...
        */
        if(tabInfo && tabInfo.componentRenderCount === 0 
            && this.props.currentPage === localConstant.supplierpo.EDIT_VIEW_SUPPLIER_PO_CURRENTPAGE){
                this.props.actions.FetchVisitDocOfSupplierPo();
        }
        
        // this.props.actions.FetchSupplierPODocumentTypes();
    ...
    fileupload=(files)=>{
            ...
            if (documentFile.length > 0) {
                ...
                const assignmentDocType = this.props.supplierPODocumentTypeData;
                ...
                if (failureFormat.length === 0 && filesToBeUpload.length > 0) {
                    for (let i = 0; i < filesToBeUpload.length; i++) {
                        ...
                        this.updatedData = {
                            ...
                            documentuniqueIdentifier: Math.floor(Math.random() * (Math.pow(10, 5))),
                            status: "C",
                        ...
                        this.props.actions.AddSupplierPODocumentDetails(createdNewRecords);
                        this.updatedData = {};
                        this.uniqueName = {};
                        createdNewRecords = [];
                    }
                ...
                if (failureFiles.length > 0) {
                    IntertekToaster(failureFiles.toString() + localConstant.documents.FILE_LIMIT_EXCEDED, 'warningToast assignmentDocSizeReq');
                }

            }
            else {
                IntertekToaster(localConstant.documents.NO_FILE_SELECTED, 'warningToast assignmentNoFileSelectedReq');
            }
    }
                ...
                if (response[i] !== null) {
                    ...
                    if (!response[i].status) {
                        ...
                        updatedDataAfterUploaded = {
                            ...
                            moduleCodeReference: !this.props.interactionMode ? 0 : this.props.supplierPONumber,
                            canDelete: true,
                            isUploaded: true
                        };
                    }
                    this.props.actions.UpdateSupplierPODocumentDetails(updatedDataAfterUploaded, this.UploadedFile[0]);
                }
            }
        }
        else {
            const afterRemovefailedDoc = this.props.documentrowData.filter(x => x.documentName !== uniqueNameDetail.documentName);
            this.props.actions.UpdateSupplierPODocumentDetails(afterRemovefailedDoc);
        }
  ...
  customerVisibleSwitchChangeHandler = (e,data) => {
    ...
    this.props.actions.UpdateSupplierPODocumentDetails(this.updatedData, this.editedRowData);
    this.updatedData = {};
};
    ...
    copyDocumentHandler = () => {
        ...
        const selectedSecChildRecords = this.secondChild.getSelectedRows();
        if (selectedRecords.length > 0) {
            this.copiedDocumentDetails = selectedRecords;
            IntertekToaster(localConstant.documents.SELECTED_RECORDS_COPIED, 'warningToast SupplierPoDocumentCopyMesgReq');
        }
        else if (selectedSecChildRecords.length > 0) {
            this.copiedDocumentDetails = selectedSecChildRecords;
            IntertekToaster(localConstant.documents.SELECTED_RECORDS_COPIED, 'warningToast SupplierPoDocCopyMesgReq');
        }
        else {
            IntertekToaster(localConstant.documents.SELECT_RECORDS_TO_COPY, 'warningToast SupplierPoDocumentCopyReq');
        }
    }
    pasteDocumentHandler = () => {
        ...
        if (recordToPaste.length > 0) {
            ...
            for (let i = 0; i < recordToPaste.length; i++) {
                recordToPaste[i].moduleCode = "SUPPO";
                delete recordToPaste[i].moduleRefCode;
                //recordToPaste[i].moduleRefCode = this.props.supplierPONumber;
            }
            this.props.actions.PasteDocumentUploadData(recordToPaste)
                .then(response => {
                    if (!isEmpty(response)) {
                        for (let i = 0; i < response.length; i++) {
                            selectType = this.props.supplierPODocumentTypeData.filter(Type=>{ if(Type.name === response[i].documentType)return Type; });
                            this.updatedData = {
                        ...
                        this.props.actions.AddSupplierPODocumentDetails(records);
                    }
                });
        }
        else {
    ...
    deleteContractRecord = () => {
        ...
        if (selectedRecords.length > 0) {
            ...
            if (isFileUploaded) {
                ...
                this.props.actions.DeleteSupplierPODocumentDetails(selectedRecords);
    ...
    uploadDocumentSubmitHandler = (e) => {
        ...
        const documentFile = Array.from(document.getElementById("uploadFiles").files);
            ...
            IntertekToaster(localConstant.documents.SELECT_FILE_TYPE, 'warningToast supplierPoDocFileTypeReq');
            return false;
        }
        this.fileupload(documentFile);
        // let date = new Date();
        ...
        //                             this.props.actions.AddSupplierPODocumentDetails(newlyCreatedRecords);
        //                         }
        //                     };
        //                 });
        //         }
    ...
    editDocumentHandler = (e) => {
        ...
        if (this.updatedData.documentType === "") {
            IntertekToaster(localConstant.documents.SELECT_FILE_TYPE, 'warningToast  supplierPoFileTypeReq');
            return false;
        }
        else {
            if (this.updatedData.documentType) {
    ...
    updatedtoStore=(fileName,data)=>{
        ...
        // if (parent.props.editSupplierPODocuments && parent.props.editSupplierPODocuments.recordStatus !== "N") {
        //  this.updatedData["uploadedOn"] = date;             
        //  this.updatedData["recordStatus"] = "M";
        //  } 
        parent.props.actions.UpdateSupplierPODocumentDetails(this.updatedData, data);
        this.updatedData={}; 
    }
    ...
    render() {
        ...
        bindAction(this.headerData, "EditSupplierPODocuments", this.editRowHandler);
        ...
        for(let i = 0; i < rowDocData.length; i++){
            rowDocData[i] = { ...rowDocData[i],roleBase: (this.props.interactionMode || this.props.supplierPOViewMode ) };
            ...
            <Fragment>
                <div className="customCard">
                    ...
                    <SupplierPODocumentMenuButtons
                        ...
                        viewMode={this.props.supplierPOViewMode}
                        onRef={ref => { this.child = ref; }}
                        paginationPrefixId={localConstant.paginationPrefixIds.supplierPODoc}
                    />
                </div>
                {this.state.isDocumentModalOpen &&
                    <Modal id="supplierPODocModalPopup"
                        title={this.state.isDocumentModelAddView ? localConstant.documents.UPLOAD_DOC : localConstant.documents.EDIT_DOC}
                        modalClass="supplierPODocModal"
                        ...
                        {this.state.isDocumentModelAddView ? <ModalPopupAddView
                            supplierPODocumentTypeData={supplierPODocumentTypeData}
                        ...
                        />
                            : <ModalPopupEditView
                                supplierPODocumentTypeData={supplierPODocumentTypeData}
                                handlerChange={(e) => this.formInputChangeHandler(e)}
                                editedSupplierPODocuments={this.editedRowData}
                            />}
                    </Modal>}
                { currentPage === localConstant.supplierpo.EDIT_VIEW_SUPPLIER_PO_CURRENTPAGE ?
                 <Panel colSize="s12" heading={localConstant.documents.ASSOSIATED_VISIT_DOCUMENTS}
                    className="pl-0 pr-0 pt-1 mt-3 documentPanel bold"
                    onpanelClick={this.panelClick}
                    isopen={this.state.isPanelOpen} isArrow={true}>
                    <RelatedDocuments DocumentsData={visitsDocumentsData}
                        headerData={VisitDocHeaderData}
                        interactionMode={this.props.interactionMode}
                        title={localConstant.documents.ASSOSIATED_VISIT_DOCUMENTS}
                        ...
                        paginationPrefixId={localConstant.paginationPrefixIds.supplierPOVisitDoc}
                    />
                </Panel> : null }
...
export default SupplierPoDocuments;
SupplierPoDocuments.propTypes = {
    documentrowData: PropTypes.array,
    supplierPODocumentTypeData: PropTypes.array
};

SupplierPoDocuments.defaultProps = {
    documentrowData: [],
    supplierPODocumentTypeData: []
};