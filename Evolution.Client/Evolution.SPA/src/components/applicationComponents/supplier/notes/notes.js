...
class Notes extends Component {
    ...
    notesSubmitHandler = (e) => {
        ...
        if (this.updatedData.note === undefined || this.updatedData.note === "") {
            IntertekToaster(localConstant.warningMessages.NOTE_IS_MANDATORY, 'warningToast supplierNoteReferenceTypeReq');
        } else if(editedData.supplierNoteId){ //D661 issue8
            ...
            this.props.actions.EditSupplierNotes(editedData);
            this.hideModal(e);
        }
        else{
        ...
        this.updatedData["supplierNoteId"] = Math.floor(Math.random() * (Math.pow(10, 5)));
        const supplierNotes = Object.assign([],this.props.supplierNotesData);
        supplierNotes.push(this.updatedData);
        this.props.actions.AddSupplierNotes(supplierNotes);
        this.updatedData = {};
        this.hideModal(e);
        }
    }
    ...
    noteHangleChange = (e) => {
        ...
        this.updatedData["recordStatus"] = "N";
        this.props.actions.AddSupplierNotes(this.updatedData);
        this.updatedData = {};
    }
    hideModal = (e) => {
        e.preventDefault();
    ...
    render() {
        ...
        const SupplierNotesHeaderData=HeaderData;
        const { supplierNotesData } = this.props;
        bindAction(SupplierNotesHeaderData, "ViewSupplierNotes", this.viewRowHandler);
        return (
            <Fragment>
                <CustomModal modalData={modelData} />
                <div className="customCard">
                    <h6 className="bold col s3 pl-0 pr-3">{localConstant.notes.SUPPLIER_NOTES}</h6>
                    <CardPanel className="white lighten-4 black-text" colSize="s12">
                        <ReactGrid gridRowData={supplierNotesData}
                            gridColData={SupplierNotesHeaderData}
                            handlerChange={this.noteHangleChange}
                            paginationPrefixId={localConstant.paginationPrefixIds.supplierNotes}
...
Notes.propTypes = {
    supplierNotesData: PropTypes.array
};
