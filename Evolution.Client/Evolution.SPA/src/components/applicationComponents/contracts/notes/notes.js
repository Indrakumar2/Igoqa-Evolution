...
class Notes extends Component {
    ...
    notesSubmitHandler = (e) => {
        ...
        if (this.updatedData.note === undefined || this.updatedData.note === "") {
        ...
        else if(editedData.contractNoteId){ //D661 issue8
            ...
            this.props.actions.EditContractNotesDetails(editedData);
            this.hideModal(e);
        }
        else{
    ...
    render() {
        ...
        const ContractNoteHeaderData=HeaderData;
        const { contractsNotesData } = this.props;
        bindAction(ContractNoteHeaderData, "ViewContractNotes", this.viewRowHandler);
        return (
            <Fragment>
                <CustomModal modalData={modelData} />
                <div className="customCard">
                    <h6 className="bold col s3 pl-0 pr-3">{localConstant.notes.CONTRACT_NOTES}</h6>
                    <CardPanel className="white lighten-4 black-text" colSize="s12">
                        <ReactGrid gridRowData={contractsNotesData}
                            gridColData={ContractNoteHeaderData}
                            handlerChange={this.noteHangleChange}
                            paginationPrefixId={localConstant.paginationPrefixIds.contractNotes}
                            onRef={ref => { this.child = ref; }} />
                       {this.props.pageMode!==localConstant.commonConstants.VIEW &&  <NotesAddButton uploadNoteHandler={this.addNoteHandler}  
                       interactionMode={this.props.interactionMode}/> }
                    </CardPanel>
                    {this.state.isShowNoteModal &&
                    <Modal id="contractNoteModalPopup"
                        title={'Notes'}
                        modalClass="contractNoteModal"
                        buttons={this.state.isNoteModelAddView?this.noteAddButtons:this.NoteViewButtons}
                        isShowModal={this.state.isShowNoteModal}>
                         <NoteModalPopup
                                name={localConstant.notes.CONTRACT_NOTES}
...
Notes.propTypes = {
    contractsNotesData: PropTypes.array
};
